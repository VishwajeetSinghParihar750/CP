...
int ans(int lvl, int nums)
{
    ...
    int p = POWER(10ll, lvl);

    int v = sum[lvl - 1] * (nums / p) + p * (nums / p * (nums / p + 1)) / 2;

    int left = nums % p;

    v += left * (nums / p + 1);

    return v + ans(lvl - 1, left - 1);
}
...
void solve()
{
    ...
    int lvl = 0, cnt = 0, val = 9;

    for (; cnt + val <= k; lvl++, cnt += val, val *= 10)
    {
        v += sum[lvl];
    }
    int left = (k - cnt) / (lvl + 1);
    int md = (k - cnt) % (lvl + 1);
