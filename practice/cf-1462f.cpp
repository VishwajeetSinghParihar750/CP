...
void solve()
{
    ...
    for (auto &[cl, cr] : seg)
    {
        int rbiggerthanmyl = r.end() - lower_bound(r.begin(), r.end(), cl);

        int rbiggerbutlalsobigger = l.end() - upper_bound(l.begin(), l.end(), cr);

        //
        int curans = rbiggerthanmyl - rbiggerbutlalsobigger;

        canBeKept = max(canBeKept, curans);
    }
