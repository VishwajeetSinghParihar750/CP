...
void filldp()
{
    ...
    for (int steps = 1; steps < 400; steps++)
    {
        //
        for (int i = 1; i < 33; i++)
        {

            if (lvls.back() < LLONG_MAX / 4)
            {
                lvls.pb(lvls.back() * 4 + 1);
            }

            if (steps <= 4)
                dp[i][steps]++;

            for (int k = 0; k < 4 && k <= steps; k++)
            {
                dp[i][steps] += dp[i - 1][steps - k];
...
int findactualans(int r, int k, int itr)
{
    ...
    auto it = upper_bound(lvls.begin(), lvls.end(), r);
    it--;
    int pos = it - lvls.begin();
    ...
    int retans = findactualans(r - lvls[pos], k - 1, itr + 1);
    ans += retans;
...
void solve()
{
    ...
    int rans = findactualans(r, k, 0);
    int lans = findactualans(l - 1, k, 0);
