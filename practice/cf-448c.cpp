...
int minans(int l, int r, vi &vec)
{

    auto minele = *min_element(vec.begin() + l, vec.begin() + r);

    int start = l;

    int childrenans = minele;
    ...
    for (int i = l; i < r; i++)
    {
        ...
        if (vec[i] == 0)
        {
            ...
            if (len > 0)
                childrenans += minans(start, start + len, vec);
