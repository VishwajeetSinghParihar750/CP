...
void solve()
{
    ...
    auto query = [&](int a, int v) -> int
    {
        a++, v++;
        string st = "?";
        fo(i, n - 1) st += " " + to_string(v);
    ...
    ans[n] = pos.size() + 1;
    fo(i, pos.size()) ans[pos[i]] = ans[n] - i - 1;

    int notFound = n - pos.size() - 1;

    while (notFound > 0)
    {

        int res = query(notFound, 0);

        ans[res] = ans[n] + notFound;
