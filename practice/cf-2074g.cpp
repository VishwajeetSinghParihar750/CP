...
void solve()
{
    ...
    fo(i, n) cin >> vec[i];
    //
    vec.insert(vec.begin(), vec.begin() + n - 1, vec.end());
    ...
    for (int l = 3; l <= n; l++)
    {
        for (int start = 0; start <= sz - l; start++)
        {
            //
            int ending = start + l - 1;

            int prod = vec[start] * vec[ending];

            dp[start][ending] = max(dp[start][ending], dp[start + 1][ending]);

            for (int i = start + 1; i < ending; i++)
            {

                dp[start][ending] = max(dp[start][ending], dp[start][i] + dp[i + 1][ending]);

                int curans = prod * vec[i];
                //
                curans += (dp[start + 1][i - 1] + dp[i + 1][ending - 1]);
                //

                dp[start][ending] = max(dp[start][ending], curans);
            }

            ans = max(ans, dp[start][ending]);
        }
    }
    println(ans);
}
