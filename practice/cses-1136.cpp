...
bool isAncestor(int u, int v)
{
    ...
    return ((u == -1) or (timein[u] <= timein[v] and timeout[u] >= timeout[v]));
}
...
void solve()
{
    ...
    for (int p = 1; p <= 20; p++)
    {
        for (int i = 1; i <= n; i++)
        {
            if (parents[i][p - 1] != -1 and parents[parents[i][p - 1]][p - 1] != -1)
                parents[i][p] = parents[parents[i][p - 1]][p - 1];
            else
                parents[i][p] = -1;
        }
    }
