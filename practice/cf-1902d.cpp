...
void solve()
{
    ...
    fo(iter, q)
    {
        ...
        if (posofpoint.count({x, y}))
        {
            vi &vec = posofpoint[{x, y}];
        ...
        pii prevpos = curpoint[l - 1];
        pii endpos = curpoint[r];

        int xdif = x - prevpos.first, ydif = y - prevpos.second;

        pii tocheckpos = {endpos.first - xdif, endpos.second - ydif};

        bool works = true;
        if (posofpoint.count(tocheckpos))
        {
            vi &vec = posofpoint[tocheckpos];
            auto it = lower_bound(vec.begin(), vec.end(), l);

            if (!((it != vec.end()) and *it <= r))
                works = false;
        }
        else
            works = false;
