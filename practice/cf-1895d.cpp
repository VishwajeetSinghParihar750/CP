...
struct Node
{
    vector<int> kids; // for 0 and 1
    Node() : kids(2, -1) {}
};

vector<Node> t(1); //  0th node doesnt do anything

void addNode(bitset<32> prexr)
{
    int curnode = 0;
    for (int i = 31; i >= 0; i--)
    {
        ...
        if (t[curnode].kids[who] == -1)
        {
            t[curnode].kids[who] = t.size();
            t.emplace_back();
        }

        curnode = t[curnode].kids[who];
    }
}

int findMaxXor(bitset<32> start)
{
    //
    int cur = 0;
    ...
    for (int i = 31; i >= 0; i--)
    {
        if (t[cur].kids[1 - start[i]] != -1)
        {
            ans |= (1 << i);

            cur = t[cur].kids[1 - start[i]];
        }
        else if (t[cur].kids[start[i]] != -1)
        {
            cur = t[cur].kids[start[i]];
        }
        else
            break;
    }
