...
void solve()
{
    ...
    //
    int crowpos = 0, time = 0;

    time = vec[0]; // first crow goes bakc
    vec[0] = 0;
    crowpos = k;

    for (int i = 1; i < n and crowpos <= l; i++)
    {

        int mannewpos = 0;
        if (vec[i] > crowpos)
            mannewpos = max(vec[i] - time, crowpos);
        else
            mannewpos = min(vec[i] + time, crowpos);

        if (crowpos >= mannewpos and crowpos - mannewpos < k)
        {
            crowpos = mannewpos + k;
        }
        else
        {
            if (mannewpos > crowpos)
            {

                int inctime = min(mannewpos - crowpos, (crowpos - k - vec[i - 1]) + (mannewpos - crowpos) / 2);
