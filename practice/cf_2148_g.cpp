...
void solve()
{
    ...
    unordered_map<int, int> zp;                   // total cnti
    unordered_map<int, tuple<int, int, int>> nzp; // keeps min power and its cnt , total cnt for a prime
    ...
    fo(i, n)
    {
        ...
        for (auto it = nzp.begin(); it != nzp.end();)
        {

            auto &[mnp, c, tc] = it->second;
            if (!cp.contains(it->first))
            {
            ...
            else
            {
                ...
                if (cp[it->first] < mnp)
                    it->second = {cp[it->first], 1, tc + 1};
                else if (cp[it->first] == mnp)
                    it->second = {mnp, c + 1, tc + 1};
                else
                    it->second = {mnp, c, tc + 1};

                auto [_, cc, __] = it->second;
                ans[i] = max(ans[i], i - cc + 1);
            }
