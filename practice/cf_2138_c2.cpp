...
void solve()
{
    ...
    {
        cin >> inp;
        adj[inp - 1].pb(i + 1), adj[i + 1].pb(inp - 1);
    }
    //
    ...
    auto dfs = [&](auto self, int v, int p, int d = 1) -> void
    {
        dcnt[d]++;
        for (auto i : adj[v])
        {
    ...
    int buf = n - accumulate(dcnt.begin(), dcnt.begin() + ld + 1, 0ll);

    //
    map<int, int> cnt;
    fo(i, ld + 1) cnt[dcnt[i]]++;
    cnt.erase(0);
    ...
    for (auto [a, b] : cnt)
    {
        ...
        for (int v = 0; v <= k; v++)
        {
            for (int i = a, c = 1; c <= b && v >= i; c++, i += a)
            {
                ndp[v] = min(ndp[v], dp[v - i]);
            }
        }
