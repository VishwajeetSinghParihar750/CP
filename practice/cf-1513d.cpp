...
void solve()
{
    ...
    sort(order.begin(), order.end(), [&](int ai, int bi)
         { return vec[ai] < vec[bi]; });

    vector<bool> mark(n - 1, false); // edge that starts from that node
    ...
    for (auto i : order)
    {
        ...
        int l = i, r = i;
        while ((l - 1 >= 0) and (vec[l - 1] % vec[i] == 0) and (!mark[l - 1]))
            l--;

        while ((r + 1 < n) and (vec[r + 1] % vec[i] == 0) and (!mark[r]))
            r++;
