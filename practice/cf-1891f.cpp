...
int findResult(int v, int tl, int tr, int pos)
{
    ...
    else
    {
        ...
        if (pos >= tl and pos <= mid)
            return findResult(2 * v, tl, mid, pos);
        else
            return findResult(2 * v + 1, mid + 1, tr, pos);
    }
}
...
void update(int v, int tl, int tr, int l, int r, int change) // this is change
{
    ...
    if (tl == l && tr == r)
    {
        t[v] += change; // for addition size needs to be multiplied
    }
    else
    {
        pushChanges(v);
...
void pointUpdate(int v, int tl, int tr, int pos, int newvalue) // this is change
{

    if (tl == pos && tr == pos)
    {
        t[v] = newvalue; // for addition size needs to be multiplied
    }
    else
    {
        pushChanges(v);
        ...
        if (pos >= tl and pos <= mid)
            pointUpdate(2 * v, tl, mid, pos, newvalue);
        else
            pointUpdate(2 * v + 1, mid + 1, tr, pos, newvalue);
    }
}

vvi adj;
vi flattree, tin, tout;
int timer = 0;
...
void solve()
{
    ...
    // reset
    timer = 0;
    flattree.clear(), tin.assign(sz + 1, 0), tout.assign(sz + 1, 0);
    ...
    for (auto i : qrs)
    {
        if (i.size() == 1)
        {
            ...
            rem[addedthis] = findResult(1, 0, n - 1, tin[addedthis]);
        }
        else
        {
            int v = i[0], x = i[1];
    ...
    for (int i = 1; i <= n; i++)
    {
        int ans = findResult(1, 0, n - 1, tin[i]);
        ans -= rem[i];
        print(ans);
    }
