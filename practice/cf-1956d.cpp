...
void makeAllIncreasing(int till, int start);
void makeAllSame(int what, int start);
...
void makeAllSame(int what, int start)
{
    ...
    makeAllIncreasing(what - 1, start);

    ops.pb({start, start + what - 1});
}
...
void makeAllIncreasing(int till, int start)
{
    ...
    makeAllSame(till, start + 1);

    for (int i = start + 1; i < start + till; i++)
    {
        ops.pb({i, i}); // making it 0
    }

    makeAllIncreasing(till - 1, start);
}
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (bestmask & (1 << i))
        {
            ...
            {
                if (vec[j] != 0)
                    ops.pb({j, j}); // making it 0
            }

            makeAllSame(cnt, start);
        }
    }

    print(bestans), println(ops.size());

    fo(i, ops.size()) print(ops[i].first + 1), println(ops[i].second + 1);
}
int32_t main()
{
#ifndef ONLINE_JUDGE
