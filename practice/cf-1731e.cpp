...
void fillpre()
{
    ...
    for (int i = 2; i < NN / 2 + 5; i++)
    {
        if (smallestprime[i] == i)
            for (int j = i * i; j < NN / 2 + 5; j += i)
            {
    ...
    for (int i = 2; i < NN / 2 + 5; i++)
    {
        ...
        while (n > 1)
        {
            int curprime = smallestprime[n];
            primes.pb(curprime);

            while (smallestprime[n] == curprime)
                n /= curprime;
        }
        //
        int sz = primes.size();
...
void solve()
{
    ...
    for (int i = n / 2; i > 1; i--)
    {
        //
        int till = n / i;
        // gcd i will be using is i, so i-1 atleast needed

        int howmany = f[till] / (i - 1);

        int maxhowmany = m / (i - 1);

        int used = min(howmany, maxhowmany);
