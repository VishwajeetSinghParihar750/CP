...
void solve()
{
    ...
    int swapcost = 1e12, dltcost = 1e12 + 1;

    int zero = 0, one = inf, two = inf;
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        int newzero = cntOne * dltcost;

        int newone = inf, newtwo = inf;
        if (s[i - 1] == '1')
        {
            newone = min(newone, min(zero, one + dltcost));

            newtwo = min(newtwo, min(two, one));
        }
        else
        {
            newone = min(newone, one + swapcost);

            newtwo = min(newtwo, two + dltcost);
        }

        zero = newzero, one = newone, two = newtwo;
    }

    println(min(zero, min(one, two)));
}
int32_t main()
{
#ifndef ONLINE_JUDGE
