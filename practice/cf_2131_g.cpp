...
vector<mint> score(60), scoreTill(60);
vi ops(60), opsTill(60);
void pre()
{

    score[1] = scoreTill[1] = ops[1] = opsTill[1] = 1;
    //
    for (int i = 2; i < 60; i++)
    {
        //
        ops[i] = opsTill[i - 1] + 1;
        score[i] = scoreTill[i - 1] * i;
        opsTill[i] = opsTill[i - 1] + ops[i];
        scoreTill[i] = scoreTill[i - 1] * score[i];
    }
}
...
void solve()
{
    ...
    mint ans = 1;
    while (!a.empty() && a.back() < 60 && ops[a.back()] <= k)
    {
        ans *= score[a.back()];
        k -= ops[a.back()];
        a.pop_back();
    ...
    while (k > 0)
    {
        ...
        if (k > 0)
        {
            int till = upper_bound(opsTill.begin(), opsTill.end(), k) - opsTill.begin() - 1;
