...
void solve()
{
    ...
    fo(i, n)
    {
        ...
        bestdh[units[i][0]] = max(bestdh[units[i][0]], units[i][1] * units[i][2]);
    }
    //
    cin >> m;

    vi bestpowerforcost(C + 1);

    for (auto [c, dh] : bestdh)
    {
        for (int k = c, mult = 1; k <= C; k += c, mult++)
        {
            bestpowerforcost[k] = max(bestpowerforcost[k], dh * mult);
        }
    }

    fo(i, C) bestpowerforcost[i + 1] = max(bestpowerforcost[i], bestpowerforcost[i + 1]);
    ...
    fo(i, m)
    {
        ...
        auto it = upper_bound(bestpowerforcost.begin(), bestpowerforcost.end(), mons[i].first * mons[i].second);

        if (it == bestpowerforcost.end())
            print(-1);
        else
            print(it - bestpowerforcost.begin());
    }
    println("");
}
int32_t main()
