...
void solve()
{
    ...
    sz.assign(n + 1, 0), parent.assign(n + 1, 0);

    //
    vvi edges(n - 1, vi(3));

    fo(i, n - 1) cin >> edges[i][1] >> edges[i][2] >> edges[i][0];
    ...
    fo(i, n - 1)
    {
        ...
        int sza = sz[findSet(u)], szb = sz[findSet(v)];

        ans = (ans * POWER((k - wt + 1) % MOD, sza * szb - 1, MOD)) % MOD;
