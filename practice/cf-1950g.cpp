...
void solve()
{
    ...
    fo(i, mpa.size() + mpb.size()) dp[0].insert(i);
    ...
    for (int i = 1; i < 1 << n; i++)
    {
        ...
        fo(sh, n)
        {
            ...
            if (i & (1 << sh))
            {
                ...
                int toconsider = i ^ (1 << sh);

                if (dp[toconsider].count(vec[sh].first) or dp[toconsider].count(vec[sh].second))
                    dp[i].insert(vec[sh].first), dp[i].insert(vec[sh].second);
            }
        }
        if (dp[i].size())
            ans = max(ans, cnt);
