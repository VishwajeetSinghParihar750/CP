...
void solve()
{
    ...
    auto fans = [&](auto self, int l, int r, int zeropos, int zeroval) -> int
    {
        if (r < l)
            return 0;
        if (r == l)
        {
            a[r - zeropos] = zeroval + r;
            return r;
        }
        int rhsb = 32 - __builtin_clz(r);
        int lhsb = l == 0 ? 0 : 32 - __builtin_clz(l);
        // 1 based
        ...
        if (rhsb == lhsb)
        {
            ...
            return ans + self(self, l - cm, r - cm, zeropos - cm, zeroval + cm);
        }
        ...
        for (; cr <= r && cl >= l; cr++, cl--)
        {
            ...
            a[cr - zeropos] = zeroval + cl;
            a[cl - zeropos] = zeroval + cr;
        }

        if (cr == r + 1)
        {
            return ans + self(self, l, cl, zeropos, zeroval);
        }
        else
            return ans + self(self, cr, r, zeropos, zeroval);
    };

    println(fans(fans, l, r, l, 0));
    for (auto i : a)
        print(i);
    println("");
}
