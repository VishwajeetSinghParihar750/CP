...
void solve()
{
    ...
    fo(i, n / 2) aops.pb(a[n - 1 - i] - a[i]);
    fo(i, m / 2) bops.pb(b[m - 1 - i] - b[i]);
    ...
    for (int i = 0; i < maxop; i++)
    {
        ...
        if (aleft >= 2 and bleft >= 2)
        {
            if (aops[aopused] > bops[bopused])
            {
                ans += aops[aopused];
                lastaop.pb(aops[aopused]);
