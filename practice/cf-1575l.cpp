...
void update(int v, int tl, int tr, int point, int val) //
{
    ...
    else
    {
        ...
        if (point <= mid)
            update(2 * v, tl, mid, point, val);
        else
            update(2 * v + 1, mid + 1, tr, point, val);
...
void solve()
{
    ...
    //
    vi b(n);
    fo(i, n) b[i] = i + 1 - vec[i];

    //
    vpii prvec(n);
    fo(i, n) prvec[i] = {vec[i], -i};
    ...
    for (auto [_, i] : prvec)
    {
        ...
        int curans = 1 + findResult(1, 0, 2e5 + 10, 0, b[i]);

        ans = max(ans, curans);

        update(1, 0, 2e5 + 10, b[i], curans);
    }
    println(ans);
}
int32_t main()
