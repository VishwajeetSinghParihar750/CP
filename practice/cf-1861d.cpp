...
void solve()
{
    ...
    vi negprefix(n), possuffix(n);

    for (int i = 1; i < n; i++)
    {
        negprefix[i] = negprefix[i - 1] + (vec[i] >= vec[i - 1]);
    }

    for (int i = n - 2; i >= 0; i--)
    {
        possuffix[i] = possuffix[i + 1] + (vec[i] >= vec[i + 1]);
    }

    int ans = min(negprefix[n - 1] + 1, possuffix[0]);

    for (int i = 1; i < n; i++)
    {
        ans = min(ans, possuffix[i] + negprefix[i - 1] + 1);
    }
