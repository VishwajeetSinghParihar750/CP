...
void solve()
{
    ...
    //
    int zerocnt = count(vec.begin(), vec.end(), 0);
    int cur = count(vec.begin(), vec.begin() + zerocnt, 1);

    vector<mint> dp(cur + 1);
    dp[0] = 0;

    for (int bad = 1; bad <= cur; bad++)
    {
        dp[bad] = dp[bad - 1] + pqInverseMod(1ll, bad * bad, MOD);
    }

    dp[cur] *= pqInverseMod(n * (n - 1), 2ll, MOD);
