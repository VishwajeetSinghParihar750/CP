...
vi oddManacher(const string &spassed)
{
    string s = '$' + spassed + '?';
    ...
    for (int i = 1; i < n - 1; i++)
    {
        ...
        pal[i] = max(0, min(r - i, pal[mirrorpos]));
    ...
    return vi(pal.begin() + 1, pal.end() - 1);
}
...
void solve()
{
    ...
    vi odd(n), even(n);
    auto res = oddManacher(t);

    pii maxeven = {0, 0}, maxodd = {0, 0};
    ...
    fo(i, n)
    {
        ...
        int oddl = 2 * odd[i] - 1;
        if (oddl > maxodd.first)
            maxodd = {oddl, i};

        int evenl = 2 * even[i];
        if (evenl > maxeven.first)
            maxeven = {evenl, i};
    }

    if (maxeven.first > maxodd.first)
    {
        println(s.substr(maxeven.second - even[maxeven.second], maxeven.first));
    }
    else
    {
        println(s.substr(maxodd.second - odd[maxodd.second] + 1, maxodd.first));
    }
}
int32_t main()
{
