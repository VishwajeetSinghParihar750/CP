...
void solve()
{
    ...
    int cntgtz = count_if(vec.begin(), vec.end(), [](int a)
                          { return a > 0; });
    if (cntgtz == 1)
    {
        println(s);
    ...
    if (it != vec.end())
    {
        ...
        string t;
        t += ('a' + (it - vec.begin()));
        for (auto tr = vec.begin(); tr != vec.end(); tr++)
        {
            if (tr != it && *tr > 0)
            {
                t += string(*tr, 'a' + (int)(tr - vec.begin()));
            }
        }
        println(t);
    }
    ...
    {
        //
        auto st = find_if(vec.begin(), vec.end(), [](int a)
                          { return a > 0; });
        int pos = st - vec.begin();
        //
        int nxt = pos + 1;
