...
void solve()
{
    ...
    for (auto it = mp.begin(); it != mp.end(); it++)
    {
        ...
        {
            int toadd = (upper_bound(it->second.begin(), it->second.end(), it->first * it->first - i) - lower_bound(it->second.begin(), it->second.end(), it->first * it->first - i));

            if (it->first * it->first == 2 * i)
            {
                toadd--;
            }
            justforthis += toadd;
        ...
        {
            int mult = it->first * nit->first;
            if (mult > 2 * n)
                break; //

            bool swapped = false;

            if (it->second.size() > nit->second.size())
            {
                swap(it->second, nit->second);
                swapped = true;
            }

            for (auto i : it->second)
            {
                auto lb = lower_bound(nit->second.begin(), nit->second.end(), mult - i);
                auto up = upper_bound(nit->second.begin(), nit->second.end(), mult - i);
