...
void solve()
{
    ...
    fo(i, k) cin >> changes[i + 1].first;
    fo(i, k) cin >> changes[i + 1].second;
    ...
    for (int i = 1; i <= k; i++)
    {
        ...
        howmanynew[i] = (changes[i].second - changes[i - 1].second);

        if ((changes[i].first < 4 && changes[i].second != changes[i].first) ||
            (howmanynew[i] > (changes[i].first - changes[i - 1].first)))
        {
            println("NO");
            return;
        }
    ...
    {
        //
        for (int j = changes[i].first - 1; j >= changes[i].first - howmanynew[i]; j--)
        {
            //
            ans[j] = ('a' + (i + 2));
        }
