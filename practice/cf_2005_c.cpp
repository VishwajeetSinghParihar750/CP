...
void solve()
{
    ...
    auto fcheck = [&](int ind, int pos) -> pair<int, int> // ending and delta
    {
        int p = 0, n = 0;
        ...
        for (auto i : a[ind])
        {
            if (i == name[pos])
                p++, pos = (pos + 1) % 5;
            else if (name.find(i) != string::npos)
                n++;
        }
        return {pos, p - n};
    };

    vvi dp(n + 1, vi(5, INT32_MIN));
    dp[0][0] = 0;

    for (int i = 1; i <= n; i++)
    {
        fo(pos, 5) dp[i][pos] = max(dp[i][pos], dp[i - 1][pos]);

        fo(pos, 5)
        {
            if (dp[i - 1][pos] != INT32_MIN)
            {
                auto [ending, delta] = fcheck(i, pos);

                dp[i][ending] = max(dp[i][ending], dp[i - 1][pos] + delta);
            }
        }
    }
    int ans = 0;
