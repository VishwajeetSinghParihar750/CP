    ...
    else
    {
        ...
        for (auto i : adj[v])
        {
            if (i != p)
            {
                ...
                {
                    if (reserves[v][dist] == inf)
                        continue;
                    if (reserves[i][dist - 1] == inf)
                    {
                        reserves[v][dist] = inf;
                    }
                    else
                        reserves[v][dist] += reserves[i][dist - 1];
                }
            }
        }
        ...
        if (dif < 0)
        {
            for (int dist = 1; dist <= n and dif < 0; dist++)
            {
                if (reserves[v][dist] > 0)
                {
                    int toworkwith = min(abs(dif), reserves[v][dist]);

                    cost += dist * toworkwith;
                    dif += toworkwith;

                    if (reserves[v][dist] != inf)
                        reserves[v][dist] -= toworkwith;
                }
            }
