...
void dfs(int v, int p)
{
    ...
    for (auto i : adj[v])
    {
        if (i != p)
        {
            dfs(i, v);
            childsz.pb(sz[i]);
            childsumdist.pb(sumdist[i]);
        }
    }

    int total = accumulate(sumdist.begin(), sumdist.end(), 0ll);
    sz[v] += accumulate(childsz.begin(), childsz.end(), 0ll);

    int n = childsumdist.size();
    int othersadd = 0;
    fo(i, n)
    {
        // for just parent
        (ans += (childsumdist[i] + childsz[i])) %= MOD;

        //
        (ans += ((childsumdist[i] * (sz[v] - childsz[i] - 1)) % MOD)) %= MOD;

        othersadd += (childsz[i] * (sz[v] - childsz[i] - 1));

        (sumdist[v] += (childsumdist[i] + childsz[i])) %= MOD;
    }
...
void solve()
{
    ...
    else
    {
        ...
        ans = pqInverseMod(ans, ((n * (n - 1)) / 2) % MOD, MOD);
