...
void solve()
{
    ...
    auto it = find(pref.begin(), pref.end(), total);
    ...
    int maxcantake = it - pref.begin();

    // sort(c.begin(), c.end(), greater());
    //
    vector<int> taken(61, -1); // write ind of who took
    ...
    fo(i, k)
    {
        ...
        while (c[i] != 0)
        {
            ...
            // deb(c[i]);
            // deb(ind);
            if (ind == 1 && taken[2] == -1 && taken[3] != i)
                ind = 2;

            // deb(ind);

            if (f[ind] == c[i] && ind <= maxcantake)
            {
                //
                // deb("UPAR");

                if (taken[ind] == -1 && taken[ind + 1] != i)
                {
                    taken[ind] = i;
                    c[i] -= f[ind];
                }
                else if (!(ind & 1))
                {
                    //
                    // deb("AYA");
                    for (int curind = ind - 1; curind > 0; curind -= 2)
                    {
            ...
            else
            {
                ...
                else if (taken[ind] != -1)
                {
                    ...
                    int prevguy = taken[ind];

                    // deb(prevguy);

                    taken[ind] = i; // replacing

                    for (int curind = ind - 1; curind > 0; curind -= 2)
                    {
                        if ((taken[curind] != -1) || (taken[curind + 1] == prevguy or taken[curind - 1] == prevguy))
                        {
                            println("NO");
                            return;
                        }

                        taken[curind] = prevguy;
                    }
    ...
    for (int i = 2; i < 60; i++)
    {
        if ((taken[i] != -1 && taken[i - 1] == -1) || (taken[i] == taken[i + 1] && taken[i] != -1))
        {
            println("NO");
            return;
        }
