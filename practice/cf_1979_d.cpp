...
void solve()
{
    ...
    //
    cin >> n >> k >> s;
    s = string(1, s[0]) + s + s[n - 1];

    vpii wantbef(n + 1, {-1, -1}), wantaft(n + 1, {-1, -1});
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        if (i < n &&
                (wantbef[i].first == wantaft[i + 1].first && ((wantbef[i].second + wantaft[i + 1].second)) == k) ||
            (wantbef[i].first != wantaft[i + 1].first && wantbef[i].second == 0 && wantaft[i + 1].second == 0))
        {
            println(i);
            return;
        }
    ...
    if (wantbef[n] == pii(s[n] ^ 1, 0) || wantbef[n] == pii(s[n], 0))
    {
        println(n);
        return;
    }
