...
void solve()
{
    ...
    n = s.size();
    //
    int cnts = count(s.begin(), s.end(), '0'), cntt = count(t.begin(), t.end(), '0');
    if (cnts != cntt)
    {
        println(-1);
        return;
    ...
    fo(i, n)
    {
        ...
        fors.emplace_back(s[i], i - start + 1);
    }
    reverse(all(fors)); // easier to pop
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (t[i] != fors.back().first)
        {
            ...
            auto slast = fors.back();
            fors.pop_back();

            if (fors.size())
                fors.back().second += last.second;
            else
                fors.pb(last);
