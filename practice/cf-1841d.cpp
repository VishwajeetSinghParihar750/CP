...
void solve()
{
    ...
    sort(vecr.begin(), vecr.end(), [](pii a, pii b)
    ...
    fo(i, n)
    {
        ...
        fo(j, i)
        {
            if (vecr[j].second >= vecr[i].first)
            {

                auto it = lower_bound(rs.begin(), rs.end(), min(vecr[i].first, vecr[j].first));

                int pos = it - rs.begin() - 1;
