...
vvi adj;
vpii ans; // sleep open , music open
string whichone;
...
void dfs(int v, int p)
{
    ...
    for (auto i : adj[v])
    {
        if (i != p)
        {
            ans[v].first += min(ans[i].first, ans[i].second + 1);
            ans[v].second += min(ans[i].second, ans[i].first + 1);
        }
    }
    if (whichone[v] == 'P')
        ans[v].first = inf;
    else if (whichone[v] == 'S')
        ans[v].second = inf;
}
...
void solve()
{
    ...
    println(min(ans[0].first, ans[0].second));
}
int32_t main()
{
#ifndef ONLINE_JUDGE
