...
void solve()
{
    ...
    fo(i, n)
    {
        dif[i] = (i > 0 ? dif[i - 1] : 0) + (s[i] == '(' ? 1 : -1);

        for (auto it = st.begin(); it != st.end() and dif[i] > 2 * it->first;)
        {

            tillthis[it->second] = i;
    ...
    int ans = 0;
    map<int, ordered_multiset<int>> mp;
    fo(i, n)
    {

        ans += mp[dif[i]].size() - mp[dif[i]].order_of_key(i);

        mp[dif[i]].insert(tillthis[i]);
    }
    println(ans);
}
int32_t main()
