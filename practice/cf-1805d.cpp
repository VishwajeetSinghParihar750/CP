...
void solve()
{
    ...
    dis1.assign(n, 0), dis2.assign(n, 0);

    // one corner
    one = true;
    dfs(0, -1);

    int start = max_element(dis1.begin(), dis1.end()) - dis1.begin();
    ...
    start = max_element(dis1.begin(), dis1.end()) - dis1.begin();
    ...
    vi till(n);
    fo(i, n) till[i] = max(dis1[i], dis2[i]);
