...
void solve()
{
    ...
    for (int i = n - 1; i >= 0; i--)
    {
        maxsuf[i] = max(maxsuf[i + 1], vec[i].second);
    }
    ...
    fo(i, n - 1)
    {
        ...
        ans = min(ans, abs(vec[i].first - maxsuf[i + 1]));

        if (!bs.empty())
        {
            auto it = bs.lower_bound(vec[i].first);
            if (it != bs.end() && *it > maxsuf[i + 1])
            {
                ans = min(ans, abs(*it - vec[i].first));
            }

            if (it != bs.begin() && (*--it > maxsuf[i + 1]))
            {
                ans = min(ans, abs(*it - vec[i].first));
            }
        }

        ans = min(ans, abs(vec[n - 1].first - vec[i].second));
        bs.insert(vec[i].second);
    }
