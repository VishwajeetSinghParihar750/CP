...
{
    unordered_map<int, int> sumofcnts;

    unordered_map<int, unordered_map<int, int>> allchildans;
    int maxone = -1, mxsiz = 0;

    for (auto i : adj[v])
    {
        if (i != p)
        {
            allchildans[i] = move(dfs(i, v));
            if (allchildans[i].size() > mxsiz)
                mxsiz = allchildans[i].size(), maxone = i;
        }
    }
    sumofcnts = move(allchildans[maxone]);
    finalans += sumofcnts[color[v]];

    allchildans.erase(maxone);

    for (auto &[i, childans] : allchildans)
    {
        for (auto &[col, cnt] : childans)
        {
            if (col != color[v])
                finalans += (cnt * sumofcnts[col]);
            else
                finalans += cnt;
            sumofcnts[col] += cnt;
        }
