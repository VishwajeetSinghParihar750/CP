...
void solve()
{
    ...
    //
    set<pii> bridges;
    vi low(n, inf), tin(n, -1);
    int timer = 0;
    //
    auto fb = [&](auto self, int v, int p) -> void
    {
        tin[v] = timer++;
        low[v] = tin[v];
        ...
        for (auto i : adj[v])
        {
            ...
            if (low[i] > tin[v])
                bridges.insert({i, v}), bridges.insert({v, i});
        }
    };
    fb(fb, 0, -1);
    ...
    bool odd = false;
    auto fans = [&](auto self, int v, int p) -> void
    {
        cv.insert(a[v]);

        for (auto i : adj[v])
        {
            if (i == p || bridges.contains({i, v}))
                continue;
