...
int findpoint(int v, int tl, int tr, int point)
{
    ...
    if (mid >= point)
        return findpoint(2 * v, tl, mid, point);

    return findpoint(2 * v + 1, mid + 1, tr, point);
}
...
void dfssolve(int v, int p)
{
    for (auto i : adj[v])
    {
        if (i != p)
        {
            int valtochange = values[v] ^ values[i];

            int szchild = sz[i];

            int changeotherthanchild = valtochange * szchild;

            update(1, 0, n - 1, 0, n - 1, changeotherthanchild);

            int changeforchildren = valtochange * (n - szchild);
            changeforchildren -= changeotherthanchild;

            update(1, 0, n - 1, tin[i], tout[i] - 1, changeforchildren);
...
void solve()
{
    ...
    fo(i, n)
    {
        ans[flattree[i]] = findpoint(1, 0, n - 1, i);
    }
    fo(i, n) print(ans[i]);
    println("");
}
