...
map<int, int> valfromthis;
void fillpre()
{
    //
    int curlog = 2;

    for (int curlog = 2; curlog < 61; curlog++)
    {
        int curl = 1ll << curlog, curr = (1ll << (curlog + 1)) - 1;
...
int findsum(int r)
{
    ...
    //
    int ans = 0;
    for (auto it = valfromthis.begin(); it != valfromthis.end() && it->first <= r; it++)
    {
        int till = next(it)->first;
        int from = it->first;
        int val = it->second;

        int range = till - from;
        range = min(range, r - from + 1);

        range %= MOD;

        (ans += ((range * val) % MOD)) %= MOD;
    }
...
void solve()
{
    ...
    int rans = findsum(r), lans = findsum(l - 1);
    int ans = (rans - lans + MOD) % MOD;
