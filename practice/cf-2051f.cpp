...
void solve()
{
    ...
    bool corneron = false, prevcorneron = false;

    pii leftrange = {1, 1}, rightrange = {n, n};

    auto checkinrange = [](pii pr, int point) -> int
    {
    ...
    fo(it, q)
    {
        ...
        if (centerexists)
        {
            int rr = checkinrange(midrange, point);
        ...
        if (corneron and prevcorneron)
        {

            int rr = checkinrange(leftrange, point);

            if (rr == 1)
                leftrange.second++;

            rr = checkinrange(rightrange, point);
        ...
        if (centerexists)
            points.insert(midrange.first), points.insert(midrange.second);

        if (corneron)
            points.insert({leftrange.first, leftrange.second, rightrange.first, rightrange.second});

        //

        prevcorneron = corneron;
