...
void solve()
{
    ...
    fo(i, n)
    {
        ...
        pos[vec[i].first][vec[i].second] = i;
    }
    ...
    fo(i, n)
    {
        int howMany = a.order_of_key(vec[i].first);

        insideMe[pos[vec[i].first][vec[i].second]] = a.size() - howMany;

        a.insert(vec[i].first);
    }
    ...
    fo(i, n)
    {
        int howMany = b.order_of_key(vec[i].second);

        containMe[pos[vec[i].first][vec[i].second]] = b.size() - howMany;

        b.insert(vec[i].second);
    }

    for (auto i : insideMe)
        print(i);
    println("");
    for (auto i : containMe)
        print(i);
    println("");
}
int32_t main()
