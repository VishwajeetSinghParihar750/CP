...
void solve()
{
    ...
    //
    int ans = 1; // for full thing

    int mex = 1;
    int l = min(a[0], b[0]), r = max(a[0], b[0]);

    ans += ((l * (l - 1)) / 2) + (((n - r - 1) * (n - r - 2)) / 2) +
           (l + (n - r - 1)); // this is for mex = 0

    if (l != r)
    {
        ans += (((r - l - 1) * (r - l - 2)) / 2) + (r - l - 1);
    }

    while (mex < n)
    {
        while (mex < n && ((b[mex] >= l && b[mex] <= r) || (a[mex] >= l && a[mex] <= r)))
        {
            //
            l = min(b[mex], l);
            l = min(a[mex], l);
        ...
        int onright = n - r;
        if (a[mex] > r)
            onright = min(onright, a[mex] - r);
        if (b[mex] > r)
            onright = min(onright, b[mex] - r);

        int onleft = l + 1;
        if (a[mex] < l)
            onleft = min(onleft, l - a[mex]);
        if (b[mex] < l)
            onleft = min(onleft, l - b[mex]);

        //

        ans += (onleft * onright);

        l = min(l, min(a[mex], b[mex])), r = max(r, max(a[mex], b[mex]));
        mex++;
    }
