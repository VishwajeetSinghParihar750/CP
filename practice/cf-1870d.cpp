...
void solve()
{
    ...
    int lastGuy = 0, lastPos = -1;
    int lastNum = 0;

    int lastMul = inf, lastMod = 0;
    fo(i, n)
    {
        //
        if (k == 0)
        ...
        if (vec[i].second > lastPos)
        {

            int thisMul = k / (vec[i].first - lastGuy);

            thisMul = min(thisMul, lastMul);

            ans[vec[i].second] = thisMul;


            k -= thisMul * (vec[i].first - lastGuy);
