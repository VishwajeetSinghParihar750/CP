...
void solve()
{
    ...
    set<pii> st{{0, s}}; // only index go in
    ...
    {
        auto top = *st.begin();
        int topitem = top.second;
        int topdist = top.first;

        st.erase(st.begin());

        for (auto p : hasprimes[topitem])
        {
            //
            if (!visited[p])
            {
                //
                for (auto item : inprime[p])
                {

                    if (dist[item] > dist[topitem] + 1)
                    {
                        dist[item] = dist[topitem] + 1;
                        parent[item] = topitem;

                        st.insert({dist[item], item});
                    }
                }
